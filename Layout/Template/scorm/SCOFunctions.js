...
function doLMSGetSuspendData() {
  ...
  /* Guarda valores na varivel _vlrSuspendData porque getSuspendData  acessvel apenas na inicializao do scorm. 
      Para manipulao dos dados guardado usar varivel.*/
...
/**
 * Pega um valor específico de um objeto JSON armazenado no suspend_data
 */
function getSuspendDataValue(key) {
    try {
        const data = JSON.parse(doLMSGetValue("cmi.suspend_data") || "{}");
        return data[key];
    } catch (e) {
        return null;
    }
...
let scormCompleted = false; // Flag para evitar múltiplas chamadas
  ...
  try {
    sections.forEach(section => {
      ...
      if (sectionTop < windowHeight && sectionTop > 0) {
        // Salva a rolagem e o ID da seção no suspend_data usando o helper para não sobrescrever
        setSuspendDataValue("scrollY", section.id);
        doLMSSetLessonLocation(window.location.href.split('#')[0]);
      }
    });

    // Marca como "completo" se estiver na última página e rolar até o final
    if (!scormCompleted && window.location.href.includes(infosToCompleteScorm.lastPageHTML) && window.scrollY + window.innerHeight >= document.documentElement.scrollHeight) {
      doLMSSetLessonStatus("completed");
      doLMSCommit();
      scormCompleted = true; // Evita disparar novamente
    }
  } catch (error) {
